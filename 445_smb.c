...
int send_weak_smb_data(const char *host, unsigned short port) {
    int sock;
    ...
    strcpy(buffer, "SMB_NEGOTIATE_REQUEST");
    send(sock, buffer, strlen(buffer), 0);
    memset(buffer, 0, sizeof(buffer));
    n = recv(sock, buffer, sizeof(buffer)-1, 0);
    if(n > 0) {
        buffer[n] = '\0';
        printf("SMB response: %s\n", buffer);
    }
    close(sock);
    return 0;
}
...
int main(int argc, char *argv[]) {
    char data[] = "Sensitive SMB Data";
    printf("Sending SMB data in a weakly protected way: %s\n", data);
    ...
    if(send_weak_smb_data(ip, port) == 0) {
        printf("Weak SMB data sent.\n");
    } else {
        printf("Failed to send SMB data.\n");
    }
