...
int send_weak_ldap_data(const char *host, unsigned short port) {
    int sock;
    ...
    strcpy(buffer, "LDAP_BIND_REQUEST");
    send(sock, buffer, strlen(buffer), 0);
    memset(buffer, 0, sizeof(buffer));
    n = recv(sock, buffer, sizeof(buffer)-1, 0);
    if(n > 0) {
        buffer[n] = '\0';
        printf("LDAP response: %s\n", buffer);
    }
    close(sock);
    return 0;
}
...
int main(int argc, char *argv[]) {
    char data[] = "Sensitive LDAP Data";
    printf("Sending LDAP data in a weakly protected way: %s\n", data);
    ...
    if(send_weak_ldap_data(ip, port) == 0) {
        printf("Weak LDAP data sent.\n");
    } else {
        printf("Failed to send LDAP data.\n");
    }
