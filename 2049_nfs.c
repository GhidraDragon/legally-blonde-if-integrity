...
int main(int argc, char *argv[]) {
    ...
    printf("Sending NFS data in a weakly protected way: %s\n", data);
    ...
    printf("UDP check for NFS port %d on %s.\n", port, ip);

    if(send_weak_nfs_data(ip, port) == 0) {
        printf("Weak NFS data sent.\n");
    } else {
        printf("Failed to send NFS data.\n");
    }
